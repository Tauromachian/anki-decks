...
- `js.txt` — JavaScript (107 cards: 42 Array incl. `Array.from / fromAsync / isArray / of`, 36 String incl. `String.fromCharCode / fromCodePoint / raw`, 9 on `var / let / const` scope + hoisting/TDZ, 9 on strict mode, 11 on regular vs arrow functions + `call / apply / bind`). Each card asks what a method/concept does with a code example.
